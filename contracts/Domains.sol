...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Ginger Name Service", "GGS") {
    tld = _tld;
  ...
  function register(string calldata name) external payable {
    require(domains[name] == address(0), "domain already exist for address");
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Ginger name service", "image": "data:image/svg+xml;base64,',
  ...
  function setRecord(string calldata name, string calldata record) external {
    require(domains[name] == msg.sender, "Can not store record for unknown address");
